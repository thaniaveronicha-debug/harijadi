...
🕋  Serta mendapat
  ...
  Future<void> _scheduleNotification(Birthday bday) async {
    if (kIsWeb) return;
    DateTime now = DateTime.now();
    DateTime next = DateTime(now.year, bday.date.month, bday.date.day, 8, 0);
  ...
  Future<void> _scanBulkImage() async {
    ...
    for (TextBlock block in recognizedText.blocks) {
      for (TextLine line in block.lines) {
        ...
        if (icReg.hasMatch(text)) {
          ...
          name = text.split(match.group(0)!)[0].trim();
        ...
        if (bday != null) {
          ...
          if (name.length > 2) detected.add(Birthday(name: name, date: bday, phoneNumber: ""));
        }
      }
    }
    ...
    if (detected.isNotEmpty) {
      ...
      if (toAdd.isNotEmpty) _confirmDetectedBirthdays(toAdd);
      else if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Tiada rekod baru ditemui.")));
    }
  }
  ...
  void _confirmDetectedBirthdays(List<Birthday> detected) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Batal')), ElevatedButton(onPressed: () { for (var b in detected) { _db.add(b.toJson()); _scheduleNotification(b); } Navigator.pop(context); }, child: const Text('Tambah Semua'))],
      ),
    );
  }
  ...
  void _showAddBirthdaySheet() {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, builder: (context) => StatefulBuilder(builder: (context, setModalState) => Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom, left: 20, right: 20, top: 10), child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Tambah Rekod', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), IconButton(icon: Icon(Icons.document_scanner, color: kIsWeb ? Colors.grey : Colors.blue), onPressed: () { if (kIsWeb) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Imbasan AI di telefon."))); } else { Navigator.pop(context); _scanBulkImage(); } })]), TextField(controller: nameController, decoration: const InputDecoration(labelText: 'Nama Penuh')), TextField(controller: phoneController, decoration: const InputDecoration(labelText: 'WhatsApp')), ListTile(title: Text("Tarikh: ${DateFormat('dd/MM/yyyy').format(selectedDate)}"), onTap: () async { final p = await showDatePicker(context: context, initialDate: selectedDate, firstDate: DateTime(1900), lastDate: DateTime.now()); if (p != null) setModalState(() => selectedDate = p); }), ElevatedButton(onPressed: () async { if (nameController.text.isNotEmpty) { final newBday = Birthday(name: nameController.text, date: selectedDate, phoneNumber: phoneController.text); final existing = await _db.where('name', isEqualTo: newBday.name).where('date', isEqualTo: newBday.date.toIso8601String()).get(); if (existing.docs.isEmpty) { _db.add(newBday.toJson()); _scheduleNotification(newBday); if (mounted) Navigator.pop(context); } else { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Rekod ini sudah wujud!"), backgroundColor: Colors.red)); } } }, child: const Text('Simpan')), const SizedBox(height: 20)])))));
  }
